...
def search_flipkart(product, total_products_count):
    try:
        word_list = product.replace(' ', '+')
        url = f'https://www.flipkart.com/search?q={word_list}&otracker=search&otracker1=search&marketplace=FLIPKART&as-show=on&as=off'
        ...
        items_div = soup.find_all('div', {'class': '_1AtVbE'})
        ...
        for count in range(loop_range):
            ...
            # Get the name
            item_name = item_div.find('a', {'class': 'IRpwTa'}).get_text()
            flipkart_products_data[count]['item_name'] = item_name.strip()

            # Get the rating
            item_rating = item_div.find('div', {'class': '_3LWZlK'})
            if item_rating:
                flipkart_products_data[count]['item_rating'] = item_rating.get_text()
            else:
                flipkart_products_data[count]['item_rating'] = 'Unavailable'

            # Get the price
            item_price = item_div.find('div', {'class': '_30jeq3 _1_WHN1'}).get_text().strip()
            flipkart_products_data[count]['item_price'] = item_price

            # Get the link
            item_link = item_div.find('a', {'class': 'IRpwTa'})['href']
            flipkart_products_data[count]['item_link'] = f'https://www.flipkart.com{item_link}'
